	...
	switch (action.type) {
		...
		case 'ADD_TO_SCHEDULE':
			if (state.schedule.includes(action.payload)) {
				...
				state.inRoom !== false &&
				state.inRoom.single_player_patient.id === action.payload.single_player_patient.id
			) {
				return {
					...state,
				...
				return {
					...
					schedule: [
						...state.schedule.filter(
							(patient) => patient.single_player_patient.id !== action.payload.single_player_patient.id
						),
						action.payload
					]
				};
